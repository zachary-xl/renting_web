  ...
  <div class="app-container">
    <el-form ref="formHeaderRef" :inline="true" :model="queryParams">
      ...
      <el-form-item label="型号名" prop="name">
        <el-input v-model="queryParams.name" @keydown.enter="getList" class="input rounded" placeholder="请输入型号名"
          clearable />
      </el-form-item>
      <el-form-item>
    ...
    <el-table :data="tableData" v-loading="loading" class="w-full" header-cell-class-name="table-header">
      ...
      <el-table-column label="充电图片" align="center" prop="chargeAvatarId">
        <template #default="{ row }">
          <el-image :src="(row as TList).chargeAvatarImage" fit="cover" class="w-[60px]">
            <template #error>
              <div class="image-slot">
    ...
    <FormComp :title="title" v-if="visible" :id="id" @confirm="onHandleConfirm"
      @update:visible="(bool) => (visible = bool)" :visible="visible" />
  </div>
</template>
<script setup lang="ts">
import { ElMessage, ElMessageBox, type FormInstance } from "element-plus";
import { Search, RefreshLeft, Plus, Picture as IconPicture } from "@element-plus/icons-vue";
...
const getList = async () => {
  ...
  for (let index in data.list) {
     ;(async function (item, index) {
       if (item!.avatarId){
         getFileDownloadAPI(item!.avatarId).then(avatarResponse=>{
           const avatarBase64String = btoa(new Uint8Array(avatarResponse.data).reduce((data, byte) => data + String.fromCharCode(byte), ""));
           tableData.value[index].avatarImage = `data:image/jpeg;base64,${avatarBase64String}`;
         })

        // try {
        //   const avatarResponse = await getFileDownloadAPI(item!.avatarId);
        //   const avatarBase64String = btoa(new Uint8Array(avatarResponse.data).reduce((data, byte) => data + String.fromCharCode(byte), ""));
        //   tableData.value[index].avatar = `data:image/jpeg;base64,${avatarBase64String}`;
        // } catch (error) {
        //   ElMessage({
        //     message: "默认图片获取失败",
        //     type: "error",
       ...
       if (item!.chargeAvatarId){
        ...
        // try {
        //   const chargeAvatarResponse = await getFileDownloadAPI(item!.chargeAvatarId);
        //   const chargeAvatarBase64String = btoa(new Uint8Array(chargeAvatarResponse.data).reduce((data, byte) => data + String.fromCharCode(byte), ""));
        //   tableData.value[index].chargeAvatar = `data:image/jpeg;base64,${chargeAvatarBase64String}`;
        // } catch (error) {
        //   ElMessage({
        //     message: "充电图片获取失败",
        //     type: "error",
