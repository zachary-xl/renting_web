<template>
  <el-dialog :before-close="cancel" :model-value="visible" :title="title" append-to-body destroy-on-close draggable
    width="680px">
        ...
        <el-col v-for="item of config" :key="item.prop" :span="item.span || 24">
          <el-form-item :prop="item.prop">
            ...
            <template v-if="item.type === 'tree-select'" #default>
              <el-tree-select v-model="formData[item.prop]" :data="item.options" :render-after-expand="false" accordion
            ...
            <template v-if="item.type === 'radio-group'" #default>
              <el-radio-group v-model="formData[item.prop]">
                <el-radio v-for="dict in item.options" :key="dict.value" :label="dict.value">{{ dict.label }}
                </el-radio>
              </el-radio-group>
            </template>
              ...
              <el-popover :width="540" placement="bottom-start" trigger="click">
                <template #reference>
                  <el-input v-model="formData[item.prop]" placeholder="点击选择图标" readonly @click="showSelectIcon">
                    <template #prefix>
                      <svg-icon v-if="formData[item.prop]" :icon-class="formData[item.prop]" class="el-input__icon"
            ...
            <template v-if="item.type === 'textarea'">
              <el-input v-model="formData[item.prop]" :autosize="{ minRows: 2 }" :placeholder="item.placeholder"
                type="textarea" />
            </template>
          </el-form-item>
        </el-col>
...
// TODO: 初始化 form, rule
const unwatchConfig = watch(() => props.config, (config) => {
...
const unwatchVisible = watch(() => props.visible, (val) => {
  console.log(val);
});
onUnmounted(() => {
  unwatchVisible();
